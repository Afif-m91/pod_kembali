...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ( $_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest' )) {
    ...
    if (isset($_GET['tgl_awal'])) {
       ...
       // $query ="SELECT * FROM stt JOIN pengirim ON stt.id_pengirim = Pengirim.id_pengirim  WHERE  (stt.id_pengirim= ?) AND (tgl_kirim BETWEEN ? AND ?) ORDER BY kd_stt ASC";
		$query ="SELECT * FROM stt JOIN pengirim ON stt.id_pengirim = Pengirim.id_pengirim  WHERE  tgl_kirim BETWEEN ? AND ? ORDER BY kd_stt ASC";
       ...
       // $stmt->bind_param("ss",$tgl_awal, $tgl_akhir );
        ...
        while ($data = $result->fetch_assoc())
            ...
            echo "<tr>
					...
					<td width='170'>".$data['alamat_penerima']."</td>
